...
function matchesTopic(item) {
  return activeTopic === "all" || item.topics.includes(activeTopic);
}
    ...
    .map(
      (item) => `
        <article class="card">
          ...
          <h3>${item.title}</h3>
          <p>${item.summary}</p>
          <p><strong>Accion:</strong> ${item.recommended_action}</p>
...
const response = await fetch("/data/radar-items.json");
